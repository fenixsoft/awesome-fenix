...
我们都知道HTTP是基于TCP协议的，必须在[TCP三次握手](https://zh.wikipedia.org/wiki/%E4%BC%A0%E8%BE%93%E6%8E%A7%E5%88%B6%E5%8D%8F%E8%AE%AE#建立通路)完成之后才能进行数据传输，这是一个通常以“百毫秒”为计时尺度的事件；此外，TCP还有慢启动的特性，使得刚刚建立连接时传输速度是最低的，后面再逐步加快到达稳定。由于TCP协议本身是面向于长时间、大数据传输来设计的，这样它连接建立的成本高昂才不至于成为瓶颈，它的稳定性和可靠性的优势才能展现出来，那显然HTTP over TCP这种搭配，在目标倾向上就产生了矛盾，以至于HTTP/1.x时代，大量短而小的TCP连接确实造成了网络性能的瓶颈。为了缓解这方面的分歧。聪明的程序员们一方面致力于减少发出的请求数量，另外一方面致力于增加客户端到服务端的连接数量，也就是上面2、3点所提到的优化措施。这些Tricks的确减少了TCP连接数量，下面两张图片是来自于[HTTP Archive](https://httparchive.org/)对最近五年来数百万个URL地址采样得出的结论，页面平均请求没有改变的情况下，TCP连接在持续减少（后面说的HTTP/2.0其实占了很大功劳）。

:::center
![](./images/http-req.png)
HTTP平均请求数量，约70余个没有明显变化
...
有了多路复用的支持，HTTP/2.0就可以对每个域名只维持一个TCP连接（One Connection Per Origin），即减轻了服务器的连接压力，开发者也不用去考虑域名分片这种事情来突破浏览器对每个域名最多6个连接的限制。

而最重要的是，没有了TCP连接数的逼迫，所有通过合并/内联文件（无论是图片、样式、脚本）以减少请求数的需求都不再需要了，甚至反而是徒增副作用的反模式了——可能还有人会反驳说：不至于吧，减少请求数量，不是至少还减少了传输中耗费的Header吗？先得承认一个事实，在HTTP协议中，Header的成本所占的比重相当的大，以至于在HTTP/2.0中需要专门考虑如何进行Header压缩的问题。但是，以下几个因素导致了通过合并资源文件减少请求数，对节省Header成本也几乎没有帮助：

- Header与Ajax（尤其是只返回少量数据的请求）相比是比重很大的开销，但与图片、样式、脚本这些静态资源相比，通常并不占主要。
- 在HTTP/2.0中Header压缩的原理是基于字典编码的信息复用（这个有机会再展开来讲），简而言之是同一个连接上产生的请求和响应越多，动态字典积累得越全，头部压缩效果也就越好。所以HTTP/2.0是单域名单连接的机制，合并资源和域名分片反而对性能提升不利。
- 与HTTP/1.x相反，HTTP/2.0本身反而变得更适合传输小资源了，譬如传输1000张10K的小图，HTTP/2.0要比HTTP/1.x快，但传输10张1000K的大图，则应该HTTP/1.x更快，这一方面是TCP连接数量（相当于多点下载）的影响，更多的是由于TCP协议丢包重传机制导致的，一个丢失的TCP包会导致所有的流都必须等待这个包重传成功，这个问题就是HTTP/3.0的解决目标了。因此，把小文件合并成大文件，在HTTP/2.0下是有害的。

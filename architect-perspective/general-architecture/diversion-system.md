...
- **内容分发网络**（Content Distribution Network）：CDN是一种十分古老而又十分透明，没什么存在感的分流系统，多出人都说听过它，但真正了解过它的人却很少。
- **负载均衡**（Load Balancing）：调度后方的多台机器，以统一的接口对外提供服务，这件事情被称为“负载均衡”。
- **缓存中间件**（Cache Middleware）：讨论数据缓存、方法缓存、进程内/外、集中式/分布式缓存等等。
...
调度后方的多台机器，以统一的接口对外提供服务，这件事情被称为“负载均衡”。

:::

互联网早期，业务流量比较小并且业务逻辑比较简单，单台服务器便可以满足基本的需求，但时至今日，互联网也好，企业也好，一般实际用于生产的系统，几乎都离不开集群了。信息系统不论是分布式单体架构还是微服务架构，不论是为了实现高可用还是为了获得高性能，都需要使用到多台机器来扩展服务能力，用户的请求不管连接到哪台机器上，都能得到相同的处理。另一方面，如何构建、调度服务集群这事情又应该对用户保持足够的透明，即使用户的请求背后是由一千台、一万台机器来共同响应的，也并非用户所关心之事，他需记住的只有一个域名地址而已。调度后方的多台机器，以统一的接口对外提供服务，这件事情被称为“负载均衡”（Load Balancing）

真正大型系统的负载均衡过程往往是多级的。譬如，在各地建有多个机房，或机房有不同网络链路入口的大型互联网站，会从DNS解析开始，通过“域名” --> “CNAME” --> “负载调度服务” --> “就近的数据中心入口”，将来访地用户根据IP（或者其他条件）分配到一个合适的数据中心中，这个过程与前一节介绍的CDN是类似的，差别是数据中心能提供的不仅是缓存，而是全方位的服务能力。由于这种方式此前已经详细介绍过，后续我们所讨论的“负载均衡”将聚焦于网络请求进入数据中心入口之后的其他级次的负载均衡。

无论在网关内部建立了多少级的负载均衡，从形式上来说可以分为两种：四层负载均衡和七层负载均衡。在详细介绍它们之前，可以先建立两个大致的概念印象：

- 四层负载均衡优势是性能高，七层负载均衡的优势功能多。
- 做多级混合负载均衡，应该是低层的负载均衡在前，高层的负载均衡在后（想一想为什么？）。

我们所说的“四层”、“七层”，指的是经典的[OSI七层模型](https://zh.wikipedia.org/wiki/OSI%E6%A8%A1%E5%9E%8B)中第四层传输层和第七层应用层，下表是来自于维基百科上对OSI七层模型的介绍，这部分属于网络基础知识就不多介绍了，我们直接从四层负载均衡具体工作过程开始说起。
...
现在所说的四层负载均衡一般都有多种工作模式，有一些模式其实是工作在二层（改写Mac地址）、三层（改写IP地址）上的，但基于习惯原因，现在都把它们统称成四层负载均衡了（部分资料上还是有二层、三层负载均衡的说法，理解是怎么回事即可），
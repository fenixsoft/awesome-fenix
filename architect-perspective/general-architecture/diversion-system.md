...
- **客户端缓存**（Client Cache）：HTTP协议的无状态性决定了它必须依靠客户端来剞劂网络传输效率的缺陷。
- **域名缓存**（DNS Cache）：DNS也许是全世界最大、使用最频繁的分布式系统，如果没有适当的分流机制，DNS将会成为整个网络的瓶颈。
...
HTTP协议的无状态性决定了它必须依靠客户端来剞劂网络传输效率的缺陷。
...
DNS也许是全世界最大、使用最频繁的分布式系统，如果没有适当的分流机制，DNS将会成为整个网络的瓶颈。

:::

讨论DNS预解析、DNS缓存通讯、失效原理和问题，HTTP DNS对比Local DNS的优点。
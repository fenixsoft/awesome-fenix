...
- **负载均衡**（Load Balance）：讨论四至七层负载均衡，均衡算法。
...
- 协议升级：绝大多数CDN提供商都同时提供SSL证书服务，可以实现源站是HTTP的，而对外开放的网站是基于HTTPS的。同理，可以实现源站是HTTP/1.x，CDN提供的外部服务是HTTPS/2.0的、实现源站是基于IPv4的，CDN提供的外部服务支持IPv6，等等。
...
- 绕过某些不存在的防御措施，这也是在国内申请CDN也必须实名备案的原因，就不细说了。

## 负载均衡

::: tip 负载均衡（Load Balance）

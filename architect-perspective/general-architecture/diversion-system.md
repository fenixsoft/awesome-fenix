...
- **缓存中间件**（Cache Middleware） <Badge text="编写中" type="warning"/>：讨论数据缓存、方法缓存、进程内/外、集中式/分布式缓存等等。
...
我们所说的“四层”、“七层”，指的是经典的[OSI七层模型](https://zh.wikipedia.org/wiki/OSI%E6%A8%A1%E5%9E%8B)中第四层传输层和第七层应用层，下表是来自于维基百科上对OSI七层模型的介绍，这部分属于网络基础知识就不多解释了（英文也懒得翻译了），我们直接从四层负载均衡具体工作过程开始说起。
...
现在所说的四层负载均衡是多种工作模式的统称，“四层”的意思是说这些工作模式都维持着同一个TCP连接，而不是说它工作在第四层。事实上，这些模式其实都是工作在二层（数据链路层，改写MAC地址）和三层（网络层，改写IP地址）上的，单纯只改第四层（传输层，难道改写端口号？）的数据无法做到负载均衡的转发，因为OSI的下三层是媒体层（Media Layers），上四层是主机层（Host Layers），既然流量都已经到主机上了，还谈什么转发，最多只能做代理了。但基于习惯和方便，现在几乎所有的资料都把它们统称为四层负载均衡，笔者同样称呼它为四层负载均衡，在这里提示一下，以免读者在某些资料上看见“二层负载均衡”、“三层负载均衡”这样的表述，误以为和这里说的四层负载均衡是一类意思。下面笔者介绍几种常见的工作模式。

### 数据链路层负载均衡

参考上面表格，数据链路层传输的内容是数据帧，譬如以常见的太网帧、ADSL宽带的PPP帧等。在我们讨论的场景中，目标肯定就是以太网帧，按照[IEEE 802.3](https://en.wikipedia.org/wiki/IEEE_802.3)标准，以最典型的1500 Bytes MTU的以太网帧结构为例：
...
帧中其他结构这里我们不去管它，只需注意到“MAC目标地址”和“MAC源地址”两项即可，我们知道每一块网卡都有独立的MAC地址，以太帧上这两个地址告诉了交换机，此帧应该是从连接在交换机上的哪个端口的网卡发出，送至哪块网卡的。

数据链路层负载均衡所做的事情，就是修改请求的数据帧中的MAC目标地址，让用户发送给负载均衡器的请求，转发到服务器集群中对应的服务器上。由于负载均衡器在数据分发过程中只修改了MAC目标地址，上层协议中的IP地址未被改变，可通过配置真实物理服务器集群所有机器虚拟IP和负载均衡服务器IP一致，从而达到不修改数据包的源地址和目的地址就可以进行数据分发的目的。正是由于实际处理请求的真实物理服务器IP和数据请求目的IP是一致的，所以响应结果就不需要通过负载均衡服务器进行地址交换，可将响应结果的数据包直接返回给用户，避免负载均衡服务器网卡带宽成为瓶颈，数据链路层的负载均衡效率也是最高的。整个请求到响应的过程如下图所示：
...
由于只有请求经过负载均衡器，而服务响应无需经过的特性，整个请求、转发、响应的链路形成一个“三角”，所以这种负载均衡模式也常被很形象地称为“三角传输模式”（Direct Server Return，DSR）、单臂模式（Single Legged Mode）或者“直接路由”（Direct Routing Mode）。

虽然数据链路层负载均衡效率很高，但它并不能应用于所有的场合，除了需要感知应用层协议信息的负载均衡它无法胜任外（所有的四层负载均衡器都无法胜任，将在后续介绍七层均衡器时一并解释），网络一侧的约束也很大，它直接改写目标MAC地址决定了它与真实的服务器必须是二层可达的，说白了就是必须在同一个子网当中，无法跨VLAN。上述优势和劣势决定了数据链路层负载均衡最适合用来做DNS均衡之后、数据中心的第一级负载均衡（注意，这里只谈负载均衡设备，并没有把[等价路由](https://zh.wikipedia.org/zh-tw/%E7%AD%89%E5%83%B9%E5%A4%9A%E8%B7%AF%E5%BE%91%E8%B7%AF%E7%94%B1)（ECMP）算进去）。

### 网络层负载均衡

根据OSI七层模型，在网络层传输的就是分组数据包，这是一种在[分组交换网络](https://zh.wikipedia.org/wiki/%E5%88%86%E7%BB%84%E4%BA%A4%E6%8D%A2)（Packet Switching）中传输的结构化数据单位。一个IP数据包由Header和Payload两部分组成，其中Header包括了24 Bytes的固定数据和最长不超过40 Bytes的可选数据组成。按照IPv4标准，一个典型的分组数据包的Header部分如下所示：
...
同样，我们对于表格中其他信息无需过多关心，只要知道在IP分组数据包的头部带有源和目标的IP地址，IP地址代表了数据是从分组交换网络中一台机器，那我们同样可以通过改变这里面的地址来实现数据包的转发。有两种很容易想到的方式，其一是保持原来的数据包不变，新创建一个数据包，把原来数据包的Header和Payload整体作为另一个新的数据包的Payload，在这个新数据包的Header中写入真实服务器的IP作为目标地址，再把这个数据包发送出去。然后在接收方设计一个拆包的机制，收到数据包后把均衡器加入的Header扔掉，还原出原来的数据包来使用，这样这台服务器就拿到了一个原本不是给它（目标IP不是它）的数据包，达到了转发的目的。大概是那时候还没有“[禁止套娃](https://www.baidu.com/s?wd=%E4%BB%80%E4%B9%88%E6%98%AF%E7%A6%81%E6%AD%A2%E5%A5%97%E5%A8%83)”的梗，只好给这种套娃式的传输起名叫做“[IP隧道](https://zh.wikipedia.org/wiki/IP%E9%9A%A7%E9%81%93)”（IP Tunnel）。

尽管需要封装新的数据包，IP隧道的转发模式比起直接路由模式效率有所下降，但由于并没有修改原有数据包中的Header，IP隧道的转发模式仍然具备三角传输的特性，即均衡器转发来的请求，可以由真实服务器直接应答，无需在经过均衡器。而且由于IP隧道工作在网络层，所以可以跨越VLAN，因此摆脱了直接路由模式中网络侧的约束。此模式从请求到响应的过程如下图所示：
...
而这种方式的缺点是它要求服务器必须得支持[IP隧道协议](https://en.wikipedia.org/wiki/Encapsulation_(networking))（IP Encapsulation）协议，这个其实并非什么大问题，但多少与“透明”这个原则有冲突。而另外一个更大的冲突是必须通过专门的配置保证所有的真实服务器与均衡器有着相同的虚拟IP地址，因为回复该数据包时，需要使用这个虚拟IP作为响应数据包的源地址，这样客户端收到这个数据包时才能正确解析。

对服务器进行虚拟IP的配置并不是在任何情况下都可行的，尤其是当有好几个服务公用一台物理服务器的时候。此时就需要考虑另一种改变目标数据表的方式：直接把数据包Header中的目标地址改掉，这样原本由用户发给均衡器的数据包，也会发送到真实服务器手上，而且没有经过IP隧道的包装，也就不需要拆包了。但现在问题是由于这种模式修改了目的IP地址，这样如果服务器直接将应答包发给客户端的话，其源IP是应用服务器的IP，客户端就无法正常处理这个应答。因此，必须让流量继续回到负载均衡，负载均衡将应答包的源IP改回自己的IP再发到客户端，这样才可以保证正常通信。如果你对网络常识稍微有了解的话，会很容易想到这不就是在家里、公司、学校上网时，由一台路由器带着一群内网机器上网的“[网络地址转换](https://zh.wikipedia.org/wiki/%E7%BD%91%E7%BB%9C%E5%9C%B0%E5%9D%80%E8%BD%AC%E6%8D%A2)”（Network Address Translation，NAT）操作吗？此时就是负载均衡器充当了这台上网路由器的作用，只要你的机器将自己的网关地址设置为均衡器地址，就无需再进行任何设置了。这种负载均衡的模式的确就被称为是NAT模式，此模式从请求到响应的过程如下图所示：
...
NAT模式的负载均衡会带来较大的（比起直接路由和IP隧道模式，往往是数量级上的下降）性能损失，这点是显而易见的，由负载均衡器代表整个服务集群来进行应答，各个服务器的响应数据都会互相挣抢均衡器的出口网络资源，这就如同在家里上网如果由人在下载，你打游戏可能就会觉得卡是一个道理，此时整个系统的瓶颈很容易就出现在负载均衡器上。

还有一种更加彻底的NAT模式，均衡器在转发时，不仅修改目标地址，连源地址也一起改了，改成均衡器自己的IP，称作Source NAT（SNAT）。这样好处是服务器连网关都无需配置了，可以让应答流量经过正常的三层路由回到负载均衡器上，做到了彻底的透明。但是缺点是由于做了SNAT，服务器处理请求时就无法拿到客户端的真实IP地址，有一些需要根据目标IP做的操作就无法进行了。

### 应用层负载均衡

前面介绍的负载均衡模式都属于转发，即直接将承载着TCP报文的底层数据（IP数据包或以太帧）转发到服务器上，此时客户端到真正响应请求的服务器维持着同一条TCP通道。但工作在四层之后的模式就无法再进行转发了，只能进行代理。
...
代理这个词，根据“哪一方能感知到”，可以分为正向代理、反向代理和透明代理。正向代理就是我们通常简称的代理，指在客户端设置的、代表客户端与服务器通讯的代理服务，它是客户端可知，而对服务器透明的。反向代理是指在设置在服务器这一方，代表服务器来与客户端通讯的代理服务，此时它对客户端来说是透明的。至于透明代理是指对双方都透明的，配置在网络中间设备上（譬如，在路由器上的透明翻墙代理）的代理服务。

根据以上定义，很显然，七层负载均衡器它就属于一种反向代理，如果只论网络性能，它肯定是无论如何比不过四层均衡器的，它比四层均衡器至少多一次TCP握手，有着跟NAT模式一样的带宽问题，而且通常要耗费更多的CPU。但是，如果论整个服务集群对外所体现出来的服务性能，七层均衡器并不一定会比四层均衡器差，这里面的原因就是来源于它工作在应用层，可以感知应用层通讯的内容，往往能够做出更明智的决策，玩出更多的花样来。

举个生活中的例子，四层均衡器就像银行的自助排号机，每一个达到银行的客户根据排号机的顺序，选择对应的窗口接受服务；而七层均衡器就像银行大堂经理，他会先确认客户需要办理的业务，再安排排号。这样办理理财、存取款等业务的客户，会根据银行内部资源得到统一协调处理，加快客户业务办理流程，有一些无需柜台办理的业务，甚至大堂经理直接就可以解决了（譬如，反向代理的静态资源缓存）。

代理的工作模式大家应该是比较熟悉的，因此关于七层均衡器如何工作的就不多讨论了，这里列举一些七层代理可以实现的功能，以便读者有个直观的感受：

- 前面介绍CDN应用时，所有CDN可以做的缓存类工作（就是不算CDN就近返回这种优化链路的工作），反向代理都可以实现，静态资源缓存、协议升级、安全防护、访问控制，等等。
- 七层代理可以实现更智能化的路由。譬如，根据Session路由，以实现亲和性的集群；根据URL路由，实现转职服务（如K8S Ingress均衡器）；甚至根据用户身份路由，实现对部分用户的特殊服务（如VIP服务器），等等。
- 某些安全攻击可以由七层均衡器来解决，譬如一种常见的DDoS手段是SYN Flood攻击，即黑客控制众多源客户端，使用虚假IP地址对同一目标大量发送SYN报文。从技术原理上看，由于四层均衡器无法感知协议信息，这些SYN攻击都会被转发到后端的服务器上；而七层均衡器下这些SYN攻击自然在负载均衡设备上就截止，不会影响后台服务器的正常运营。类似地，可以在七层均衡器上设定多种策略，过滤特定报文，以防御如SQL注入等应用层面的特定攻击手段。
- 多数微服务中的链路治理措施，都需要在七层中进行，譬如服务降级、熔断、异常注入等等；一台服务器只有出现物理层面或者系统层面的故障，导致无法应答TCP请求才能被四层均衡器所感知，进而剔除出去。譬如，某台服务器一直在报500错，那四层均衡器是完全无能力的。

## 缓存中间件 <Badge text="编写中" type="warning"/>

::: tip 缓存中间件（Cache Middleware）

...
> 3. 安装Kubernetes最小需要2核CPU、2GB内存。对于物理机来说，今时今日要找一台不满足的机器很困难，但对于虚拟机来说，尤其是购买云主机网站上最低配置的同学，要注意一下是否达到了最低要求。
...
预拉取镜像并不是必须的，本来初始化集群的时候就会自动拉取Kubernetes中要使用到的Docker镜像组件，并且这项工作实在非常非常非常的繁琐。

但对于许多人来说这项工作往往又是无可奈何的，Kubernetes的镜像都存储在k8s.gcr.io上，如果你的机器无法直接访问gcr.io（谷歌的地址）的话，初始化集群时自动拉取就无法顺利进行，所以必须得预拉取。预拉取的意思是，由于Docker只要查询到本地有相同（名称、版本、Tag）的镜像，就不会访问远程仓库，那只要从GitHub上拉取到所需的镜像，再将Tag修改改成官方的，就可以跳过网络访问阶段。
...
这里必须使用“--kubernetes-version”参数指定具体版本，因为尽管每个版本需要的镜像信息在本地是有存储的，但如果不加的话，Kubernetes会向远程查询最新的版本号，又会导致网络无法访问的问题。

得到这些镜像名称和Tag后，可以从DockerHub上找一些仓库来拉取，具体哪些公开仓库有，可以到DockerHub网站上查询一下，比较常用的是anjia0532的仓库，有机器人自动跟官方同步，相对比较及时。
...
到了这里，终于可以安装Master节点了，使用以下命令：

```bash
$ kubeadm init --kubernetes-version v1.17.3 --pod-network-cidr=10.244.0.0/16
```

这里使用“--kubernetes-version”参数的原因与前面预拉取是一样的，另外一个参数“--pod-network-cidr”着在稍后介绍CNI网络插件时再去说明。看到下面信息之后，说明集群主节点已经安装完毕了。

```
Your Kubernetes master has initialized successfully!
```

不过这行字后面，还会有三行“you need……”、“you should……”、“you can……”开头的内容，这是三项可选的工作，下面继续介绍。

#### [可选] 让非Root用户可以使用Kubernetes

Kubernetes是以root用户安装的，如果需要非root的其他用户也可以使用Kubernetes集群，那需要为该用户配置admin.conf文件。操作如下：
...
第一种方案，尤其不在GCP或者AWS的云主机上，没有它们的命令行管理工具时，需要大量的手工配置，基本上是反人类的。实际通常都会采用第二种方案，使用CNI插件来处理容器之间的网络通讯，所以本节所标识的“[可选]”其实也没什么选择的余地。

Kubernetes目前支持的CNI插件有：Calico、Cilium、Contiv-VPP、Flannel、Kube-router、Weave Net等六种，每种网络提供了不同的管理特性（如MTU自动检测）、安全特性（如是否支持加密通讯）、网络策略（如Ingress、Egress规则）、传输性能（甚至对TCP、UDP、HTTP、FTP、SCP等不同协议来说也有不同的性能表现）以及主机的性能消耗。后续我们将专门对不同CNI插件进行测试对比，在环境部署这部分，对于初学者来说，使用Flannel是最合适的，它是最精简的CNI，没有安全特性的支持，主机压力小，安装容易，效率也不错，使用以下命令安装：
...
这部分内容是告诉用户，集群的Master节点已经建立，其他节点的机器可以使用“kubeadm join”命令加入集群。这些机器只要完成kubeadm、kubelet、kubectl的安装即可、其他的所有步骤，如拉取镜像、初始化集群等等都不需要去做，就是可以使用该命令加入集群了。需要注意的是，该Token的有效时间为24小时。